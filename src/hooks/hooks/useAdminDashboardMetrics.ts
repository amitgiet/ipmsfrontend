  ...
  const fetchMetrics = async () => {
    try {
      ...
      const { data: sprints, error: sprintsError } = await supabase
        .from('sprints')
        .select('id, project_id, status, end_date');
      ...
      const projectsPlanned = projects?.filter(p => p.project_status?.toLowerCase() === 'planned').length || 0;
      const projectsRunning = projects?.filter(p => p.project_status?.toLowerCase() === 'in-progress').length || 0;
      const projectsOnHold = projects?.filter(p => p.project_status?.toLowerCase() === 'on-hold').length || 0;
      const projectsCompleted = projects?.filter(p => p.project_status?.toLowerCase() === 'completed').length || 0;
      ...
      setMetrics({
        ...
        projectsOnHold,
        projectsCompleted,
        projectsWithoutSprints,
        projectsNeedPlanning,
        runningSprints,
        overdueSprints,
        unhealthySprints,
        totalResources,
