...
export const useProductOwnerProjects = (currentUser: any) => {
  ...
  const fetchMyProjects = async () => {
    try {
      console.log('🔄 Fetching projects for product owner:', currentUser?.email);
      setLoading(true);
      ...
      // Find the team member with product_owner role
      ...
      if (!teamMemberData) {
        console.log('❌ No product owner found with email:', currentUser.email);
        toast({
          title: "Access Denied", 
          description: `No product owner found with email ${currentUser.email}. Please contact administrator.`,
          variant: "destructive",
        });
        setProjects([]);
        setLoading(false);
      ...
      console.log('✅ Found product owner:', teamMemberData);
      ...
      if (!assignmentData || assignmentData.length === 0) {
        console.log('❌ No projects assigned to this product owner');
        setProjects([]);
        setLoading(false);
        return;
      }
